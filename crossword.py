...
class Crossword(object):
    ...
    def getGrid(self, max_tries):
		if not word_elements[0] return None
        for tries in range(max_tries):
            clear() # always start with a fresh grid and char_index
            # place the first word in the middle of the grid
            var start_dir = randomDirection()
            var r = floor(len(grid) / 2)
            var c = floor(len(grid[0]) / 2)
            var word_element = word_elements[0]
            if (start_dir == "across"){
                c -= floor(len(word_element.word)/2)
            } else {
                r -= floor(len(word_element.word)/2)
            }
    ...
    this.getLegend = function(grid){
        ...
        var position = 1
        for(var r = 0; r < len(grid); r++){	
            for(var c = 0; c < len(grid[r]); c++){
                var cell = grid[r][c]
    ...
    var placeWordAt = function(word, index_of_word_in_input_list, row, col, direction){
        if(direction == "across"){
            for(var c = col, i = 0; c < col + len(word); c++, i++){
                addCellToGrid(word, index_of_word_in_input_list, i, row, c, direction)
            }
        } else if(direction == "down"){
            for(var r = row, i = 0; r < row + len(word); r++, i++){
    ...
    var canPlaceWordAt = function(word, row, col, direction){
        # out of bounds
        if(row < 0 || row >= len(grid) || col < 0 || col >= len(grid[row])) return false

        if(direction == "across"){
            # out of bounds (word too long)
            if(col + len(word) > len(grid[row])) return false
            # can't have a word directly to the left
            if(col - 1 >= 0 && grid[row][col - 1] != None) return false
            # can't have word directly to the right
            if(col + len(word) < len(grid[row]) && grid[row][col+len(word)] != None) return false
            ...
            for(var r = row - 1, c = col, i = 0; r >= 0 && c < col + len(word); c++, i++){
                var is_empty = grid[r][c] == None
                var is_intersection = grid[row][c] != None && grid[row][c]['char'] == word.charAt(i)
            ...
            for(var r = row + 1, c = col, i = 0; r < len(grid) && c < col + len(word); c++, i++){
                var is_empty = grid[r][c] == None
                var is_intersection = grid[row][c] != None && grid[row][c]['char'] == word.charAt(i)
                var can_place_here = is_empty || is_intersection
                if(!can_place_here) return false
            ...
            # out of bounds
            if(row + len(word) > len(grid)) return false
            # can't have a word directly above
            if(row - 1 >= 0 && grid[row - 1][col] != None) return false
            # can't have a word directly below
            if(row + len(word) < len(grid) && grid[row+len(word)][col] != None) return false
            ...
            # current word
            for(var c = col - 1, r = row, i = 0; c >= 0 && r < row + len(word); r++, i++){
                var is_empty = grid[r][c] == None
                var is_intersection = grid[r][col] != None && grid[r][col]['char'] == word.charAt(i)
                var can_place_here = is_empty || is_intersection
                if(!can_place_here) return false
            }

            # same deal, but look at the column to the right
            for(var c = col + 1, r = row, i = 0; r < row + len(word) && c < len(grid[r]); r++, i++){
                var is_empty = grid[r][c] == None
                var is_intersection = grid[r][col] != None && grid[r][col]['char'] == word.charAt(i)
                var can_place_here = is_empty || is_intersection
                if(!can_place_here) return false
    ...
    var findPositionForWord = function(word){
        ...
        if(len(bests) == 0) return false

        # find a good random position
        var best = bests[floor(random()*len(bests))]
    ...
    # seems to work well
    word_elements.sort(function(a, b){ return len(b.word) - len(a.word); })
}
